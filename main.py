...
#     """)
#
#     cur.execute("INSERT INTO cars VALUES(NULL, 'Запарожец',1000)") #добавим строчку вниз таблицы
#     last_row_id = cur.lastrowid #содержит айди последней записи
#     buy_car_id = 2
...
# data = [('car','машина'), ('house', 'дом'), ('color', 'цвет')]
#
...
if __name__ == '__main__':
    ...
    # print(session.query(Lesson).all()) #запрос данных из группы Lesson.в виде списка
    # print("*" * 50)
    #
    ...
    # print("*" * 50)
    # for it in session.query(func.count(Student.surname),
    #                         Group.group_name).join(Group).\
    #         group_by(Group.group_name).having(func.count(Student.surname) < 25):
    #     print(it)
    ...
    # for it in session.query(Student).filter(text("age Like '1%'")).order_by(text('name, id desc')):
    #     print(it)
...
# per = Person("Игорь", 25)
#
# tm = Template("Мне {{ p.age }} лет. Меня зовут {{ p.name}}")   #экземпляр класса
# # tm = Template("Мне {{ p['age'] }} лет. Меня зовут {{ p['name']}}")   #экземпляр класса
# msg = tm.render(p=per)
#
#
